...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			DrawCircleV(transform.position, 15.f, VIOLET); // Rysuj plazmę jako niebieską kulę
		}
	}
	Vector2 GetPosition() const {
		return transform.position;
	...
	float GetRadius() const {
		if (type == WeaponType::BULLET) {
			return 5.f; // Promień dla pocisku
		}
		else if (type == WeaponType::LASER) {
			return 2.f; // Promień dla lasera
		}
		else if (type == WeaponType::PLASMA) {
			return 8.f; // Promień dla plazmy
		}
		return 0.f; // Domyślny przypadek
	}
	...
	float speed)
{
	...
	if (wt == WeaponType::LASER) {
		...
		return Projectile(pos, vel, 40, wt);
	}
	return Projectile(pos, vel, 0, wt); // Domyślny przypadek
}
...
public:
	...
	void Heal(int amount) {
		if (!alive) return;
		hp += amount;
		if (hp > 100) hp = 100; // Maksymalne HP
	}
	...
	float GetFireRate(WeaponType wt) const {
		switch (wt) {
		case WeaponType::LASER: return fireRateLaser;
		case WeaponType::BULLET: return fireRateBullet;
		case WeaponType::PLASMA: return 8.f; // wolniejsza
		default: return 0.f;
		}
	}
	...
	float GetSpacing(WeaponType wt) const {
		switch (wt) {
		case WeaponType::LASER: return spacingLaser;
		case WeaponType::BULLET: return spacingBullet;
		case WeaponType::PLASMA: return 60.f;
		default: return 0.f;
		}
	}
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						// If PLASMA {HP++}
						if ((*pit).GetType() == WeaponType::PLASMA) {
							player->Heal(1);
						}
